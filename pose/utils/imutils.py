...
def load_image(img_path):
    # H x W x C => C x H x W
    return im_to_torch(np.array(imageio.imread(img_path)))

def door_load_image(img_path, img_res):
    # H x W x C => C x H x W
    img=imageio.imread(img_path)
    img = cv2.resize(img, dsize=(img_res, img_res), interpolation=cv2.INTER_CUBIC)
    return im_to_torch(np.array(img))

def resize(img, owidth, oheight):
    print('%f %f' % (img.min(), img.max()))
    img = cv2.resize(img, dsize=(oheight, owidth), interpolation=cv2.INTER_LINEAR)
    # img = np.imresize(
    #         img,
    #         (oheight, owidth)
    #     )
...
def color_heatmap(x):
    ...
    #color[color > 1] = 1
    color = (color/np.max(color)* 255).astype(np.uint8)
    return color
...
def sample_with_heatmap(inp, out, num_rows=2, parts_to_show=None):
    ...
    for i, part in enumerate(parts_to_show):
        part_idx = part
        out_resized = skimage.transform.resize(out[part_idx], [size, size])#cv2.resize(out[part_idx], dsize=(size, size), interpolation=cv2.INTER_LINEAR) # np.imresize(out[part_idx], [size, size])
        out_resized = out_resized.astype(float)/255
        out_img = inp_small.copy() * .3
        # print("out_resized  ", np.shape(out_resized), np.unique(out_resized))
        #color_hm = (out_resized / np.max(out_resized) *255).astype(np.uint8)
        #color_hm = color_heatmap(out_resized)
        max_output=np.max(out_resized)
        if max_output < 1e-5:
            max_output = 1e-5  

        out_resized = (out_resized / max_output *255).astype(np.uint8)
        color_hm = cv2.applyColorMap(out_resized, cv2.COLORMAP_RAINBOW)
        # plt.imshow(color_hm)
        # plt.show()
        # print("color_hmm  :  ", np.unique(color_hm), color_hm.shape)
        out_img += color_hm * .7
